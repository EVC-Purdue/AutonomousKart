...
def call(**overrides):
    ...
    speed_mps = kwargs.pop("speed_mps", 7.5)
    return pathfinder(pose[0], target, pose[1], speed_mps, **kwargs)
...
def test_straight_ahead_zero_steering_full_throttle():
    ...
    assert throttle == pytest.approx(15.0, rel=1e-6)
...
def test_curvature_regulation_slows_throttle_on_sharp_bend():
    ...
    assert t_on >= BASE["min_reg_speed_mps"] - 1e-6
...
def test_approach_distance_ramps_speed_down_near_end():
    ...
    t_ramp, _ = call(target=(0.5, 0.0), approach_dist_m=2.0, min_approach_speed_mps=0.75)
    assert t_ramp < t_no_ramp
    assert t_ramp >= 0.75 - 1e-6
...
def test_speed_mps_out_of_range_is_clamped_not_rejected():
    # Negative and over-limit inputs should still produce a sane output.
    throttle_low, _ = call(speed_mps=-0.5)
    throttle_high, _ = call(speed_mps=999.0)
    assert 0.0 <= throttle_low <= 15.0
    assert 0.0 <= throttle_high <= 15.0
