...
def pathfinder(opencv_output: Tuple, logger, logger):
    ...
    get_logger.info(f"Theta1: {theta1:.1f}°, Theta2: {theta2:.1f}°")
    get_logger.info(f"Desired Heading: {desired_heading:.1f}°")
    get_logger.info(f"Target Speed: {target_speed} mph | Current Speed: {speed:.1f} mph")
    get_logger.info(f"Steering Command: {steering_command:.1f}° {'Left' if steering_command < 0 else 'Right' if steering_command > 0 else 'Straight'}")
    ...
    Dummy
    motor_speed = 7 #20 * random.random()
    steering_angle = 7 #180 * random.random() - 90
